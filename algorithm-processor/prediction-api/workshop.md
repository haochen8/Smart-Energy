...
3. **Test load distribution:**
   ...
   for i in {1..5}; do
     curl -X POST http://localhost:30080/predict \
       -H "Content-Type: application/json" \
       -d "{\"AREA\": \"load-test-$i\", \"historical_data\": [{\"timestamp\": \"2025-01-01T00:00:00\", \"value\": $((100+i))}]}"
   done
   ```
...
1. Make several API calls with different areas
2. Connect to Redis and analyze the data structures:
   ...
   - `areas:active` (set) - Active area tracking
